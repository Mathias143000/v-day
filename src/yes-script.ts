...
window.addEventListener("load", () => {
    ...
    const music = getSuccessElement<HTMLAudioElement>("bg-music")
    music.volume = 0.3
    void music.play().catch(() => {})
    successMusicPlaying = true

    const toggle = getSuccessElement<HTMLButtonElement>("music-toggle")
    toggle.textContent = "🔊"
    toggle.setAttribute("aria-label", "Выключить музыку")
})

function getSuccessElement<T extends HTMLElement>(id: string): T {
...
function toggleSuccessMusic(): void {
    const music = getSuccessElement<HTMLAudioElement>("bg-music")
    const toggle = getSuccessElement<HTMLButtonElement>("music-toggle")
